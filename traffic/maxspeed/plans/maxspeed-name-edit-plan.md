...
> ⚠️ **SUPERSEDED — họp Phụng 15/06/2026** (`~/Downloads/MapOp Team Discuss.docx`). Bản dưới đây (17 operator, 680M, scope motorway→tertiary 118.570 km) **không còn là kế hoạch chốt**. Thay đổi:
> - **Đơn vị công việc: theo cell → theo tuyến đường** (quốc lộ/cao tốc, chỉ motorway/trunk/primary ≈ **28.000 km**; bỏ tertiary + tỉnh lộ; bỏ trung tâm Hà Nội).
> - **Tags chốt:** maxspeed + cấm vượt (no-overtaking) + biển ra/vào khu dân cư. Mỗi task một tag. Tên đường: sai → sửa, thiếu → bỏ qua.
> - **Bắt buộc evidence/edit:** mỗi edit phải có link ảnh street-view trong changeset, nếu không cộng đồng revert + ban account. Layer tham khảo (MaxBit/maxspeed.nl, Grab-style editor) chỉ để tra cứu.
> - **Ngân sách part-time tính sau (measure-then-budget):** Tuệ+Quân tự edit + quay màn hình → SOP → thuê ~20-30 editor part-time (trả theo km, ~100k đ/100km TBD). Phải đo **thời gian/edit** trước.
>
> Giữ lại bản dưới làm tham chiếu số liệu OSM. Số liệu chốt mới: xem `dashboard` road-list + memory `project_scope_pivot_2026_06_15`.
