...
- `traffic/signs/` — the **O3KR2 sign-detection pipelines** that feed the Editor Spatial DB. `mapillary_signs.py` (moved from `coverage/prep/06`) pulls Mapillary Map Features over VN → `[lat,lng,value,object_value,sign_id,first_seen,last_seen]`; today it fetches only the **speed-limit** sign family and writes to the coverage data lake (`admin-poi/coverage/data/mapillary_sign_points.parquet`). GSV branch + residential/no-overtaking classes are still to add.
