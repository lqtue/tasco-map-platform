...
## Setup
```bash
python3.13 -m venv admin-poi/coverage/.venv
admin-poi/coverage/.venv/bin/pip install -r coverage/requirements.txt
```
...
$PY admin-poi/coverage/prep/01_admin.py            # -> data/admin_wards.parquet  (34 prov, 13 đặc khu)

# 2. Open Buildings via GEE  (needs an Earth Engine Cloud project)
EE_PROJECT=<id> $PY admin-poi/coverage/prep/02_gee_buildings.py            # launch 34 Drive exports
EE_PROJECT=<id> $PY admin-poi/coverage/prep/02_gee_buildings.py --monitor  # poll
#   then download the Drive folder OB_VN_BUILDINGS into admin-poi/coverage/data/buildings/

# 3. OSM highways  (source: traffic/maxspeed/baseline/vietnam-latest.osm.pbf)
$PY admin-poi/coverage/prep/03_osm_roads.py        # -> data/roads.parquet  (built / building)

# 4. Assemble master cell table
$PY admin-poi/coverage/prep/04_build_cells.py      # -> data/cells.parquet + data/cells_res9.parquet
