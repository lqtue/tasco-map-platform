...
Run:  admin-poi/coverage/.venv/bin/streamlit run dashboards/app.py

Data sources (read-only):
  - traffic/maxspeed/baseline/maxspeed_coverage_result.json  (maxspeed coverage)
  - traffic/maxspeed/baseline/name_coverage_result.json      (street-name, tertiary+)
  - traffic/maxspeed/baseline/name_coverage_by_province.json (street-name by province)
  - traffic/maxspeed/baseline/name_coverage_full_result.json (all classes incl resi)
  - admin-poi/coverage/data/cells.parquet                             (H3 res-10 master cells)
  - admin-poi/coverage/data/road_coverage_cells.parquet               (per-cell maxspeed/name km)
  - dashboards/data/sat_envelope.json         (precomputed buy km²)
Tracking state is written to dashboards/data/progress.json.
"""
import json
...
# ponytail: reorg split the roots — traffic/ stayed at the true repo root,
# coverage data moved under archive/. Point each at where it actually lives.
TRUE_ROOT = Path(__file__).resolve().parents[2]
BASELINE = TRUE_ROOT / "traffic/maxspeed/baseline"
COVERAGE_DATA = REPO / "admin-poi/coverage/data"
...
# Display grid: source parquets are res-10 (~16,470 m², ~5M cells, too many to
# read/click). Roll up to a coarser parent at load — the res-10 parquets stay
# the source of truth, this is a derived display grid (set to 10 for a no-op).
DISPLAY_RES = 8
# class rank for "strongest road in this cell"; mirrors 05_road_coverage.py:37
...
def _pick_mode(df, col):
    """Most-common non-null value of `col` per h3_id, vectorised (groupby-size,
    not a python lambda — ~20x faster over millions of rows)."""
    return (df.dropna(subset=[col]).groupby(["h3_id", col]).size()
              .reset_index(name="n").sort_values("n")
              .drop_duplicates("h3_id", keep="last")[["h3_id", col]])


def _rollup(df, res):
    """Aggregate res-10 cells to their res-`res` H3 parent. Metrics sum; ratios,
    centre, and area recompute against the parent cell. Canonical Uber H3 ids."""
    df = df.copy()
    df["h3_id"] = [h3.cell_to_parent(c, res) for c in df["h3_id"]]
    df["_rank"] = df["top_class"].map(RANK)               # class → int for fast max
    num = {
        "built_up_area_m2": "sum", "building_count": "sum",
        "road_km": "sum", "maxspeed_km": "sum", "name_km": "sum", "lanes_km": "sum",
        "is_island": "any", "is_island_land": "any",
        "road_built": "any", "road_construction": "any", "_rank": "max",
    }
    num = {k: v for k, v in num.items() if k in df.columns}
    g = df.groupby("h3_id", as_index=False).agg(num)
    g["top_class"] = g.pop("_rank").map({v: k for k, v in RANK.items()})
    for col in ("province", "ward"):
        if col in df.columns:
            g = g.merge(_pick_mode(df, col), on="h3_id", how="left")
    g["cell_area_m2"] = [h3.cell_area(c, "m^2") for c in g["h3_id"]]
    latlng = [h3.cell_to_latlng(c) for c in g["h3_id"]]
    g["lat"] = [p[0] for p in latlng]
    g["lng"] = [p[1] for p in latlng]
    g["res9_id"] = [h3.cell_to_parent(c, res - 1) for c in g["h3_id"]]
    g["built_up_ratio"] = (g["built_up_area_m2"] / g["cell_area_m2"]).clip(0, 1)
    return g


@st.cache_data
def load_signs():
    """Mapillary speed-limit sign points binned to the display H3 resolution.
    Empty frame if the feed hasn't been pulled yet (prep/06_mapillary_signs.py)."""
    p = COVERAGE_DATA / "mapillary_sign_points.parquet"
    if not p.exists():
        return pd.DataFrame(columns=["h3_id", "sign_count", "sign_speeds"])
    pts = pd.read_parquet(p).dropna(subset=["lat", "lng"])
    pts["h3_id"] = [h3.latlng_to_cell(la, lo, DISPLAY_RES)
                    for la, lo in zip(pts["lat"], pts["lng"])]
    return (pts.groupby("h3_id").agg(
        sign_count=("sign_id", "count"),
        sign_speeds=("value", lambda s: sorted({int(v) for v in s.dropna()})),
    ).reset_index())
...
def load_cells():
    ...
    if "top_class" not in base:
        base["top_class"] = None
    base = _rollup(base, DISPLAY_RES)
    base = base.merge(load_signs(), on="h3_id", how="left")
    base["sign_count"] = base["sign_count"].fillna(0).astype(int)
...
def signal_count(d):
    """How many of the four speed-limit signals a cell has evidence for:
    law-default (any tertiary+ road → a legal default always exists),
    existing maxspeed tag, Mapillary street-view sign, satellite (placeholder)."""
    law = (d["road_km"] > 0).astype(int)
    ms = (d["maxspeed_km"] > 0).astype(int)
    sv = (d["sign_count"] > 0).astype(int)
    sat = 0  # no satellite inference yet — column reserved for Sprint 2+
    return (law + ms + sv + sat)


def cell_colors(d, colorby, pct):
    """Per-row RGB for the H3 layer; either a heat ramp or criteria priority."""
    if colorby == "Signal coverage":
        return _heat(signal_count(d) / 4.0, [237, 248, 233], [0, 68, 27])  # greens
...
with tab_map:
    ...
    if not has_road:
        st.warning("`admin-poi/coverage/data/road_coverage_cells.parquet` missing or empty — "
                   "run `admin-poi/coverage/prep/05_road_coverage.py` to light up the maxspeed/name layers. "
    ...
    colorby = ctl[5].radio("Colour by", ["Criteria", "Signal coverage", "Built-up heat",
                                         "Maxspeed-gap heat", "Name-gap heat"])
    ...
    if len(sel):
        ...
        slim = pd.DataFrame({
            ...
            "signs": render["sign_count"].astype(int).values,
            "n_sig": signal_count(render).values,
        })
        ...
        st.pydeck_chart(pdk.Deck(
            ...
            tooltip={"html": "<b>{province}</b> — {ward}<br/>"
                             ...
                             "road {road_km_r} km · maxspeed {ms_km_r} · name {name_km_r}<br/>"
                             "signals {n_sig}/4 · street-view {signs} sign(s)"}))
    else:
...
""")
    with st.expander("📐 Methodology — maxspeed coverage"):
        st.markdown(f"""
**Source.** `traffic/maxspeed/baseline/run.sh`: Geofabrik Vietnam PBF → `osmium tags-filter` to tertiary+ highways → per-way length via pyproj `Geod.line_length` (**geodesic WGS84 km**).
