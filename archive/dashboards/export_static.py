...
+ baseline JSONs) and writes  dashboards/static/data.js  as
`window.DATA = {...}` (a .js, not .json, so index.html can load it via file://).

Run:  admin-poi/coverage/.venv/bin/python dashboards/export_static.py
"""
import json
...
# the sat-img purchase demo was relocated out of archive/ to traffic/sat-imagery/
OUT = TRUE_ROOT / "traffic/sat-imagery/data.js"

THRESHOLDS = [0.0, 0.05, 0.10, 0.20, 0.30]
MAP_RES = 7  # ~5 km²/cell — coarse enough for a snappy national overview

# Road-class cutoff for the "strategic road" criterion, ordered most→least major.
# idx = max class rank included, so cutoff 1 = "Trunk+" = motorway+trunk, etc.
CUTOFFS = ["motorway", "trunk", "primary", "secondary", "tertiary"]
CUTOFF_LABELS = ["Motorway", "Trunk+", "Primary+", "Secondary+", "Tertiary+"]
CLASS_RANK = {c: i for i, c in enumerate(CUTOFFS)}
...
def province_table(base, provinces):
    ...
    `buy_grid[ti][ci]` is the buy-envelope km² when the urban bar is THRESHOLDS[ti]
    AND the strategic-road criterion is class-cutoff CUTOFFS[ci] (rank <= ci). The
    road criterion uses each cell's dominant `top_class` (a per-cell proxy — the
    parquet has no per-class km split), so e.g. Primary+ ≈ cells whose main road is
    primary-or-bigger. `road_km2_by_cut[ci]` is just the road-corridor area at that
    cutoff (threshold-independent)."""
    idx = {p: i for i, p in enumerate(provinces)}
    nT, nC = len(THRESHOLDS), len(CUTOFFS)
    rows = [{"ms_gap": 0.0, "name_gap": 0.0, "road_km": 0.0, "island_km2": 0.0,
             "buy_km2": [0.0] * nT, "urban_km2": [0.0] * nT,
             "buy_grid": [[0.0] * nC for _ in range(nT)],
             "ur_km2": [[0.0] * nC for _ in range(nT)],   # urban∪road, no island
             "road_km2_by_cut": [0.0] * nC}
            for _ in provinces]

    rank = base["top_class"].map(CLASS_RANK)
    has_road = base["road_km"].fillna(0) > 0
    area = base["cell_area_m2"]

    def by_prov(mask):
        return base.assign(a=area * mask).groupby("prov")["a"].sum() / 1e6
    ...
    for p, v in by_prov(base["is_island_land"]).items():
        rows[idx[p]]["island_km2"] = round(float(v), 1)

    road_at = [has_road & (rank <= ci) for ci in range(nC)]
    for ci in range(nC):
        for p, v in by_prov(road_at[ci]).items():
            rows[idx[p]]["road_km2_by_cut"][ci] = round(float(v), 1)

    for ti, thr in enumerate(THRESHOLDS):
        urban = base["built_up_ratio"] >= thr
        # legacy fields (the VN demo reads these): operational primary+ road set
        buy = urban | base["road_built"] | base["road_construction"] | base["is_island_land"]
        for p, v in by_prov(buy).items():
            rows[idx[p]]["buy_km2"][ti] = round(float(v), 1)
        for p, v in by_prov(urban).items():
            rows[idx[p]]["urban_km2"][ti] = round(float(v), 1)
        for ci in range(nC):
            for p, v in by_prov(urban | road_at[ci] | base["is_island_land"]).items():
                rows[idx[p]]["buy_grid"][ti][ci] = round(float(v), 1)
            for p, v in by_prov(urban | road_at[ci]).items():
                rows[idx[p]]["ur_km2"][ti][ci] = round(float(v), 1)
    return rows
...
def hexes(base, provinces):
    ...
    # dominant (most major) road class present in the hex; NaN where road-less
    rank = base["top_class"].map(CLASS_RANK)
    agg["cls"] = base.assign(rank=rank)[base["road_km"].fillna(0) > 0] \
        .groupby("res7")["rank"].min()
    agg["isl"] = base.groupby("res7")["is_island_land"].max()
    ...
    for h, r in agg.iterrows():
        ...
        feats.append({
            ...
            "properties": {
                ...
                "c": int(r["cls"]) if pd.notna(r["cls"]) else -1,
                "i": int(bool(r["isl"])),
                "p": int(idx[r["prov"]]),
            },
        })
...
def main():
    ...
    # national network km included, cumulative by class cutoff (class + its _link)
    cls = ms["classes"]
    ckm = lambda c: cls.get(c, {}).get("total_km", 0) + cls.get(c + "_link", {}).get("total_km", 0)
    class_km, run = [], 0.0
    for c in CUTOFFS:
        run += ckm(c)
        class_km.append(round(run))
