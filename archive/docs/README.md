...
- 🏗️ **[Enterprise Architecture](./vision/enterprise-architecture.md)**: kiến trúc đích 4 tầng (Sources → Foundation → Serving → Frontend); giải nghĩa **"Editor Spatial DB Server" = Temporal Spatial DB** (Postgres+PostGIS versioned). Nguồn: [okr-ea-2026.06.pdf](./vision/okr-ea-2026.06.pdf).
...
**🟢 Enrichment (Tuệ).** tertiary+ = 133,771 km, **chỉ 12.9% có maxspeed** → thiếu 116,498 km. Lấp: lớp luật (Quân, ~100%, $0 ngoài) + ảnh verify. Sat buy-envelope **20,350 km²**. Street view ~2 tháng/~15 TB. Dashboard: `../dashboards/app.py`. → [data-scope](./leadership/2026-06-10-data-scope.md), [brief](./leadership/2026-06-10-brief.md).
...
- [tasco-mobility-platform.md](./vision/tasco-mobility-platform.md) · [okr-2026.06.md](./vision/okr-2026.06.md) (+ pdf, png) · [enterprise-architecture.md](./vision/enterprise-architecture.md) (+ [okr-ea-2026.06.pdf](./vision/okr-ea-2026.06.pdf))
- [road-attribute-editor-plan.md](./vision/road-attribute-editor-plan.md) — product plan for the internal multi-source editor (`traffic/lanes/`): the plain 5-step flow, what we're building & how it grows beyond maxspeed (attribute × evidence matrix), the team/roles it takes, then the estimation method in depth (8 sub-problems), phased roadmap, decisions
...
- maxspeed pitch deck (13 slide 16:9): phủ tốc độ ~28k km quốc lộ như một bài toán ước lượng; nguồn = [road-attribute-editor-plan](./vision/road-attribute-editor-plan.md)
  - 🇻🇳 [maxspeed-pitch.html](./leadership/maxspeed-pitch.html) / [.pdf](./leadership/maxspeed-pitch.pdf) · 🇬🇧 [maxspeed-pitch-en.html](./leadership/maxspeed-pitch-en.html) / [.pdf](./leadership/maxspeed-pitch-en.pdf)
...
**Code & module** (ngoài docs/): [`../traffic/`](../traffic/) (enrichment: [`maxspeed/`](../traffic/maxspeed/) baseline+dashboard+plans + multi-product `dashboards/` + research + [`wiki/`](../wiki/) luật→OSM tag → Confluence) · [`../admin-poi/coverage/`](../admin-poi/coverage/) (H3 imagery planner) · [`../admin-poi/geocode/`](../admin-poi/geocode/) (dataset reverse-geocode admin VN current+past, 2025 merger) · [`../CLAUDE.md`](../CLAUDE.md) (chỉ dẫn repo)
...
- **Maxspeed/sat số** → reproduce: `../traffic/maxspeed/baseline/run.sh`, `../dashboards/`.
