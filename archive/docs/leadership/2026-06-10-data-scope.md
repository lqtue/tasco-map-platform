...
**Người chuẩn bị:** Tuệ · **Cho:** anh Phụng present · **Nguồn số:** `traffic/maxspeed/baseline/maxspeed_coverage_result.json` (OSM extract 2026-06-06) + `admin-poi/coverage/data/cells.parquet`.
...
---
*Reproduce: `traffic/maxspeed/baseline/run.sh` cho maxspeed; query `admin-poi/coverage/data/cells.parquet` cho km² (script trong lịch sử chat). Street-view = mô hình tham số, chỉnh overlap/MB-per-km/tài xế để ra lại bảng.*
